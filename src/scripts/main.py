...
def main():
    ...
    log_reg_model.to_onnx(Path.cwd().parent / "models" /"onnx_models" / "log_reg_model.onnx")
    ...
    random_forest_model.to_onnx(Path.cwd().parent / "models" /"onnx_models" / "random_forest_model.onnx")

    lib_ext = ".dll" if os.name == "nt" else ".so"
    random_forest_model.to_treelite(
        lib_path=Path.cwd().parent / "models" / "treelite_models" / f"random_forest_model{lib_ext}"
    )
    ...
    svm_model.to_onnx(Path.cwd().parent / "models" /"onnx_models" / "svm_model.onnx")
